...
app.post('/submit-order', upload.fields([
  ...
  try {
    console.log("🟠 FULL DEBUG: Received raw req.body:", req.body);
    console.log("🟠 FULL DEBUG: Received req.files:", req.files);
    ...
    const uploadImages = async (files, prefix) => {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (error) {
          ...
          urls.push(`https://easuceuubzkidoykxfsu.supabase.co/storage/v1/object/public/photofoiy-uploads/${path}`);
        }
      }
      return urls;
    };
